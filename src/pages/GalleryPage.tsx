...
const GalleryThumb: React.FC<{ 
    ...
    const isUploading = item.videoStatus === 'uploading';
    const isProcessing = item.videoStatus === 'processing' || item.videoStatus === 'queued';
    const isFailed = item.videoStatus === 'failed';

    return (
        <div 
            onClick={() => !isUploading && onClick(item)} 
            className={`group relative aspect-[9/16] overflow-hidden bg-white/5 border border-white/10 transition-all duration-300 rounded-lg backdrop-blur-sm
            ${isUploading ? 'border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.3)] cursor-wait' : 
              isProcessing ? 'border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.3)] cursor-wait' : 
              isVideoReady ? 'border-green-500/30 hover:border-green-400 shadow-[0_0_5px_rgba(34,197,94,0.2)] hover:shadow-[0_0_20px_rgba(34,197,94,0.4)] cursor-pointer' : 
              'hover:border-purple-500 hover:shadow-[0_0_20px_rgba(168,85,247,0.4)] cursor-pointer'}`}
        >
            ...
            <img 
                ...
                className={`w-full h-full object-cover transition-all duration-1000 ${!isUploading && 'group-hover:scale-110'} ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} 
            />

            {/* 3. UPLOADING LOCK OVERLAY (Priority UI) */}
            {isUploading && (
                <div className="absolute inset-0 bg-black/70 backdrop-blur-[2px] z-30 flex flex-col items-center justify-center animate-fadeIn">
                    <div className="relative mb-3">
                        <div className="w-10 h-10 border-4 border-white/10 rounded-full" />
                        <div className="absolute inset-0 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin" />
                    </div>
                    <span className="text-white text-[10px] font-heading tracking-widest uppercase font-bold text-center px-2 animate-pulse">Uploading...</span>
                    <span className="text-indigo-300 text-[8px] font-mono mt-1 text-center tracking-wider">Syncing to Drive</span>
                </div>
            )}

            {/* 4. VIDEO STATUS INDICATOR (LOG VISUAL) */}
            {isVideoMode && !isUploading && (
                <>
...
const GalleryPage: React.FC<GalleryPageProps> = ({ 
  ...
  const itemsRef = useRef<GalleryItem[]>([]); // Track items for effect closure access
  ...
  // --- ADAPTIVE DATA POLLING & DELTA SYNC ---
  useEffect(() => {
     let timerId: ReturnType<typeof setTimeout>;
     let isMounted = true;

     // Reset cursor when event changes
     cursorRef.current = 0;
     ...
     const loadGallery = async () => {
        ...
        // 1. Pause if hidden
        if (document.hidden) {
            console.log("[POLL] Gallery Fetch Paused (Hidden)");
            timerId = setTimeout(loadGallery, 10000);
            return;
        }

        let nextInterval = 15000; // Idle speed

        try {
            // INCREMENTAL FETCH
            const response = await fetchGallery(activeEventId, cursorRef.current);
            const { items: newItems, nextCursor } = response;

            if (newItems && newItems.length > 0) {
                console.log(`[GALLERY] Fetched ${newItems.length} items (Since: ${cursorRef.current})`);
                
                // MERGE STRATEGY
                setItems(prevItems => {
                    const mergedMap = new Map();
                    // 1. Put old items in map
                    prevItems.forEach(i => mergedMap.set(i.id, i));
                    // 2. Put/Overwrite new items in map (Updates status or adds new)
                    newItems.forEach(i => mergedMap.set(i.id, i));
                    
                    // 3. Convert back to array
                    const mergedArray = Array.from(mergedMap.values()) as GalleryItem[];
                    
                    // 4. SORTING PRIORITY: Active Tasks First, Then Date
                    return mergedArray.sort((a, b) => {
                        // Check if items are active (processing, queued, uploading)
                        const activeStates = ['processing', 'queued', 'uploading'];
                        const aActive = activeStates.includes(a.videoStatus || '');
                        const bActive = activeStates.includes(b.videoStatus || '');

                        // Active items float to top
            ...
            // Update Cursor if backend returned a newer one
            ...
            // 2. Adaptive Logic: Check if any items are active
            const currentItems = itemsRef.current; // Access latest state via Ref
            const hasActiveTasks = newItems.some(i => i.videoStatus === 'processing' || i.videoStatus === 'queued' || i.videoStatus === 'uploading') || 
                                   currentItems.some(i => i.videoStatus === 'processing' || i.videoStatus === 'queued' || i.videoStatus === 'uploading');
            
            if (hasActiveTasks) {
                nextInterval = 4000; // Fast refresh
                console.log(`[POLL] Gallery Active. Next: 4s`);
            } else {
                console.log(`[POLL] Gallery Idle. Next: 15s`);
            }
        ...
        if (isMounted) {
            timerId = setTimeout(loadGallery, nextInterval);
        }
     };

     loadGallery(); // Initial call
  ...
  // Sync state to parent cache when items change
  useEffect(() => {
      if (items.length > 0) {
          // Filter items visually but cache everything relevant
          const filteredItems = items.filter(i => i.type !== 'video' || i.videoStatus === 'done' || i.videoStatus === 'ready_url' || i.videoStatus === 'uploading');
          onUpdateCache(filteredItems);
      }
  }, [items, onUpdateCache]);

  // Display items are filtered
  const displayItems = items.filter(i => i.type !== 'video' || i.videoStatus === 'done' || i.videoStatus === 'ready_url' || i.videoStatus === 'uploading' || i.videoStatus === 'processing' || i.videoStatus === 'queued');
  ...
  const executeDelete = async () => {
      ...
      try {
          ...
          // Rollback implicitly handled by next poll if persistent
      } finally {
          setIsDeleting(false);
      }
  };
  ...
  const executeClearGallery = async () => {
      ...
      try {
          ...
          setItems([]);
          onUpdateCache([]);
          cursorRef.current = 0; // Reset cursor
  ...
  const handleGenerateVideoFromGallery = async () => {
      ...
      try {
         ...
         if (res.ok) {
             // Optimistic Update
             setItems(prev => prev.map(i => i.id === selectedItem.id ? { ...i, videoStatus: 'queued' } : i));
             setSelectedItem(prev => prev ? { ...prev, videoStatus: 'queued' } : null);
         } else {
  ...
  const handleItemClick = (item: GalleryItem) => {
      // Prevent clicking on uploading items
      if (item.videoStatus === 'uploading') return;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-xl p-4 md:p-8 animate-[fadeIn_0.2s]" onClick={() => !showConceptSelector && !showQRModal && !showDeleteConfirm && setSelectedItem(null)}>
           <div className="relative w-full h-full max-w-6xl max-h-[90vh] flex flex-col bg-black rounded-2xl overflow-hidden border border-white/10 shadow-2xl" onClick={e => e.stopPropagation()}>
               <div className="relative flex-1 w-full h-full overflow-hidden flex items-center justify-center bg-black">
                  ...
                  {viewMode === 'result' && (selectedItem.videoStatus === 'processing' || selectedItem.videoStatus === 'queued' || selectedItem.videoStatus === 'uploading') && settings?.boothMode === 'video' && (
                      <div className="absolute top-24 left-6 bg-blue-900/80 backdrop-blur px-6 py-3 rounded-full border border-blue-500/30 flex items-center gap-3 z-30 shadow-[0_0_20px_rgba(37,99,235,0.3)]">
                         ...
                         <span className="text-xs md:text-sm text-white font-bold uppercase tracking-[0.2em] animate-pulse">
                             {selectedItem.videoStatus === 'queued' ? 'Video Queued...' : selectedItem.videoStatus === 'uploading' ? 'Finalizing Video...' : 'Video Rendering...'}
                         </span>
                      </div>
                  )}
                          ...
                          <div className="flex bg-black/40 backdrop-blur-md rounded-xl p-1.5 border border-white/10 shadow-lg">
                              ...
                              {settings?.boothMode === 'video' && (
                                  <>
                                      ...
                                      {(selectedItem.videoStatus === 'processing' || selectedItem.videoStatus === 'queued' || selectedItem.videoStatus === 'uploading') && (
                                           <button disabled className={`flex items-center gap-2 px-6 py-3 rounded-lg text-xs font-bold uppercase tracking-widest cursor-not-allowed ${selectedItem.videoStatus === 'uploading' ? 'bg-indigo-900/40 border border-indigo-500/50 text-indigo-200' : 'bg-blue-900/20 border border-blue-500/30 text-blue-300/50'}`}>
                                              <span className="animate-spin h-3 w-3 border-2 border-current border-t-transparent rounded-full" />
                                              {selectedItem.videoStatus === 'queued' ? 'VIDEO QUEUED' : selectedItem.videoStatus === 'uploading' ? 'UPLOADING VIDEO...' : 'VIDEO ON PROCESS'}
                                          </button>
                                      )}
